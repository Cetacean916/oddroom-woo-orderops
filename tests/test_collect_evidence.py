...
EVIDENCE_RELATIVE = Path("evidence/refinement/raw-private/step-090/backend")
...
with tempfile.TemporaryDirectory(prefix="pf07-collector-test-") as temporary:
    ...
    shutil.copy2(ROOT / "scripts/refinement_evidence_paths.py", project / "scripts/refinement_evidence_paths.py")
    ...
    (project / EVIDENCE_RELATIVE / "protected-artifact-inventory.json").write_text(
...
def execution(identity, payload_value):
    digest=payload_value["stored_payload_hash"]
    return {"execution_id":identity,"verified_authorized":True,"ingress_raw_body_sha256":digest,"verified_payload_sha256":digest,"executed_hubspot_nodes":["HubSpot 2026-03 Deal Read","HubSpot 2026-03 Deal Readback","HubSpot 2026-03 Deal Upsert"],"envelope_result":"completed","envelope_phase":"completed","envelope_deal_checkpoint_present":True,"wordpress_status":"completed","wordpress_phase":"completed","wordpress_deal_checkpoint_present":True}
...
p1=payload(1,"a@example.com","A"); p2=payload(2,"b@example.com","B"); p3=payload(3,"c@example.com","C")
trace={"schema_version":1,"fixture_id":"normal-order","orders":[
    {"order_id":1,"product_id":11,"variation_id":0,"shape":"simple","coupon_applied":False,"payload":p1,"execution":execution(101,p1)},
    {"order_id":2,"product_id":12,"variation_id":22,"shape":"variable","coupon_applied":False,"payload":p2,"execution":execution(102,p2)},
    {"order_id":3,"product_id":13,"variation_id":0,"shape":"coupon","coupon_applied":True,"payload":p3,"execution":execution(103,p3)},
...
'''
    ...
    record = json.loads((project / EVIDENCE_RELATIVE / "e2e-normal-order.json").read_text(encoding="utf-8"))
    ...
    artifact = project / EVIDENCE_RELATIVE / "artifacts/gate01_order_variety_trace.json"
    ...
    manifest_lines = (project / EVIDENCE_RELATIVE / "evidence-manifest.sha256").read_text(encoding="utf-8").splitlines()
    require(any(line.endswith(f"{EVIDENCE_RELATIVE.as_posix()}/e2e-normal-order.json") for line in manifest_lines), "transaction manifest omitted the canonical record")

    before = snapshot(project / EVIDENCE_RELATIVE)
    ...
    require(snapshot(project / EVIDENCE_RELATIVE) == before, "failed collection changed canonical evidence bytes")
