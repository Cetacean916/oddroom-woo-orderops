...
- [62초 실제 end-to-end walkthrough](https://cetacean916.github.io/portfolio-showcase/assets/media/pf07/demo-video.mp4) — 계약의 60–90초 구간 안에서 실제 상품 선택·키 입력·주문 제출과 관리자 `pending → completed` 전이, 화면에 나타나는 최종 1.0.2 패키지 worker 실행을 한 번의 연속 녹화로 보여줍니다.
...
두 영상은 정적 설명 슬라이드가 아니라 최종 Linux 1.0.2 패키지 소유 런타임의 실제 브라우저·WP-CLI·Docker Compose 동작을 연속 캡처한 결과입니다. 합성 주문과 마스킹된 별칭만 사용하며, 보호 계정·호스트·webhook·토큰·개인 식별자는 포함하지 않습니다.
...
[현재 refinement acceptance index](../evidence/refinement/public/acceptance-matrix.json)는 `GATE-01`부터 `GATE-10`까지 정확히 한 항목씩 연결합니다. 각 public record는 별도 successor 보호 record와 redaction transform을 SHA-256으로 결속하지만 보호 식별자나 raw 경로를 공개하지 않습니다. 기존 `evidence/public`은 v1.0.2 공개 기준선으로 바이트 불변 보존됩니다.
...
- [현재 refinement evidence index](../evidence/refinement/public/acceptance-matrix.json)
