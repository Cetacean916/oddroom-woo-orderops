...
The on-demand HTTPS route terminates at a pinned nginx ingress. Only the storefront and `/webhook/oddroom-orderops-v1` are routed; WordPress administration and the n8n editor remain private. nginx buffers and caps the raw request before proxying, while n8n verifies HMAC against the unchanged bytes before decoding JSON or reaching any external node. [`workflow/payload-schema-v1.json`](../workflow/payload-schema-v1.json) is the versioned Draft 2020-12 request contract. The ingress Code node mirrors its exact object-key sets, resource bounds, selected currency precision, synthetic-email boundary, signed-64-bit ID range, and event-specific rank/source rules while preserving large integers exactly; invalid authenticated input stops before every external call.
...
Hourly and manual reconciliation invoke the same seven-day, 50-order-page implementation. It scans orders deterministically without filtering on order creation time, derives creation/payment/cancellation/full-refund events from their own WooCommerce fact timestamps, inserts only a missing immutable snapshot, and uses the normal unique Action Scheduler and order-lease path. Money normalization and full-refund aggregation use precision-bounded decimal strings and arbitrary-length minor-unit arithmetic rather than binary floating point. Protected cancellation timestamps are accepted only when strict UTC parsing proves the stated calendar instant. Reconciliation never rewrites an existing payload or invents a cancellation timestamp.
...
The plugin supplies the OFFSET storefront composition without changing the active block theme. WooCommerce retains product, cart, checkout, and order semantics. The setup exposes two buyer-facing synthetic products, keeps integration fixtures out of the catalog, sets `noindex`, captures rather than sends transactional mail, and offers only a relabelled no-funds synthetic gateway. Both classic and Store API checkout force the exact `Synthetic` / `Buyer` / `pf07-demo@example.com` identity, clear address, shipping, account, and order-note fields before WooCommerce stores session data, and reject attempts to submit another identity. The classic path uses a session-only guest customer so a signed-in WordPress profile cannot be overwritten. A database-atomic limit keyed by the trusted client address and opaque WooCommerce session admits at most ten otherwise-valid attempts per 15-minute bucket, fails closed on database error, and stores only a keyed digest. Storefront activity can create synthetic demo order facts but cannot accept real customer identity, collect an address, create a customer account, or collect funds.
